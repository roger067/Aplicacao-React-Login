                  ...
                  <div style={{ marginBottom: '2rem' }}>
                    <select className="form-control" onChange={e => this.selectProduct(e.target.value)}>
                      { products.map(product => (
                        <option key={product._id} value={JSON.stringify(product)}>{product.name}</option>
                      ))}
                    </select>
                    <button type="button" className="btn" style={{ marginTop: '1rem' }} onClick={() => this.addProduct()}>Adicionar</button>
                  </div>
                    ...
                    <tbody>
                      { userProducts.map(product => (
                        <tr key={product._id}>
                          ...
                          <td><button type="button" className="btn btn-danger"><i className="glyphicon glyphicon-trash"></i></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  
                  <div className="col-sm-3 col-sm-offset-9" style={{ marginTop: '3rem' }}>
...
export default connect(mapStateToProps, { save, getById, getProducts })(UserForm);
